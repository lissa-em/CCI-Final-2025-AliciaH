...
function draw() {
  ...
  let baseHeight = 300;
  resizeCanvas(340, baseHeight + symptomHeight + 40);
  clear();
  fill(0);
  textAlign(LEFT, TOP);
  textSize(24);
...
// CSV upload
document.getElementById('analyseTasksBtn').addEventListener('click', analyseTasks);

function analyseTasks() {
  const fileInput = document.getElementById('fileInput');
  const file = fileInput.files[0];
  if (!file) {
    alert("Please select a CSV file to analyse.");
    return;
  }
  ...
  reader.onload = function(event) {
    ...
    // Get all rows 
    const dataRows = rows.slice(1).filter(row => row.length >= 3); // Ensure row has enough columns
    ...
    let EnergySum = 0, PainSum = 0;
    let EnergyCount = 0, PainCount = 0;
    
    dataRows.forEach(row => {
      const EnergyValue = parseFloat(row[2]); 
      const PainValue = parseFloat(row[3]); 
    ...
    const avgPain = PainSum / PainCount;

    console.log(`Average of column 2: ${avgEnergy}`);
    console.log(`Average of column 3: ${avgPain}`);

    // Check conditions and display result
    if (avgEnergy < 5 && avgPain > 5 || avgPain > 8 || avgEnergy < 3) {
      const resultElement = document.getElementById('result');
      text("Result: rest", 10, 10);
      resultElement.textContent = "Result: rest";
      console.log("Result: rest");
      alert("You should rest");
    } else {
      const resultElement = document.getElementById('result');
      resultElement.textContent = "Conditions not met";
      console.log("Conditions not met for 'rest'.");
      alert("You do not need to rest");
    }
  };
